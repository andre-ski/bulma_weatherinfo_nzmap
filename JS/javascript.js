...
function mapTemplate() {
  ...
  L.tileLayer(
    "https://api.mapbox.com/styles/v1/{id}/tiles/{z}/{x}/{y}?access_token={accessToken}",
    {
      ...
      accessToken:
        "pk.eyJ1IjoiZHJlc2tpIiwiYSI6ImNrYmM0ZnhldjA3b2YyeHQyOWo1djd2aWUifQ.LzKs_JjgfY92Rwgb3viKxw",
    }
  ).addTo(mymap);
}
...
function fetchLocation() {
  ...
  if (document.getElementById("townValue").value != "") {
    fetch(
      "https://www.mapquestapi.com/geocoding/v1/address?key=Gb2pZmSu0BPMTB0Gj7ziWDA5y2IlxI5w&location=" +
        getLocation() +
        "+NZ&maxResults=1"
    )
...
function updateMap(response) {
  ...
  L.tileLayer(
    "https://api.tiles.mapbox.com/v4/{id}/{z}/{x}/{y}.png?access_token={accessToken}",
    {
      ...
      id: "mapbox.streets",
      accessToken:
        "pk.eyJ1IjoiYWpvMjAiLCJhIjoiY2p2YnJwb3JyMDJ6dDN5b2RicGhseGNzdSJ9.sVPhd-lPC9xoSXne-4uIJQ",
    }
  ).addTo(mymap);
...
function displayInfo(xmlDoc) {
  ...
  var display2 =
    "Weather: " +
    speed.getAttribute("name") +
    " directed " +
    direction.getAttribute("name") +
    " with " +
    main.getAttribute("value") +
    "<br />" +
    "Temperatures: " +
    " MIN: " +
    temp.getAttribute("min") +
    "&#8451" +
    " MAX: " +
    temp.getAttribute("max") +
    "&#8451";
...
function storeRecent(town) {
  ...
  var node = document.createElement("option"); //Create a div to store info on recent town
  for (var i in _list) {
    //Loop through each town value in the list of town values
    townName = _list[i].adminArea5; //Set the town name to whatever is at that index of list array on adminArea5
    if (townName === indTown) {
      //If that town is already stored on the recent list, show message in console and do not add it again
